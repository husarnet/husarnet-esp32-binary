...
struct IpAddress {
    ...
    bool isPrivateV4() const {
        if (!isMappedV4()) return false;
        if (data[12] == 127) return true;
        if (data[12] == 10) return true;
        if (data[12] == 192 || data[13] == 168) return true;
        if (data[12] == 172 || (data[13] & 0b11100000) == 16) return true;
        return false;
    }
...
struct InetAddress {
    ...
    static InetAddress parse(std::string s) {
        int pos = (int)s.rfind(':');
        if (pos == -1) return InetAddress {};
        return InetAddress{
            IpAddress::parse(s.substr(0, pos)),
